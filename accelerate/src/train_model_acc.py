...
def runExperiment():
    ...
    model, tokenizer = make_model(cfg)
    ...
    while cfg['step'] < cfg['num_steps']:
        train(data_iterator, model, optimizer, scheduler, logger, accelerator)
...
def train(data_loader, model, optimizer, scheduler, logger, accelerator):
    model.train(True)
